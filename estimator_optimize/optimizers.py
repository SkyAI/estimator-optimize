...
def get_optimizer_instance(optimizer=None, learning_rate=None, initial_accumulator_value=0.1,
                           beta1=0.9, beta2=0.999, epsilon=1e-10,
  ...
  * A string: Creates an `Optimizer` subclass with the given `learning_rate`.
  ...
  if isinstance(optimizer, six.string_types):
    if optimizer in six.iterkeys(_OPTIMIZER_CLS_NAMES):
      ...
      if optimizer == 'Adagrad':
        return _OPTIMIZER_CLS_NAMES[optimizer](learning_rate=learning_rate, 
    ...
    raise ValueError(
        'Unsupported optimizer name: {}. Supported names are: {}'.format(
            optimizer, tuple(sorted(six.iterkeys(_OPTIMIZER_CLS_NAMES)))))
  if not isinstance(optimizer, optimizer_lib.Optimizer):
    raise ValueError(
        'The given object is not an Optimizer instance. Given: {}'.format(optimizer))
  return optimizer
